...
const NOTATION: NoteEx[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "在源码里用圆括号 `( ... )` 括住要连起来的音符。括号要**紧贴数字**写：`(5 6)` 对，`( 5 6 )` 中间带空格的会被当成别的记号，弧线**画不出来也不报错**。",
    ],
    code: "(5 6) (1' 1') 3 2 |1--- |",
  },
  {
    ...
    level: "进阶",
    body: [
      "在音符**前面**写 `{yy}` 加**延音记号**（fermata，音符上方的「◠」），表示这个音可以自由延长。",
      "写在数字**后面**（`1{yy}`）不算数，会被悄悄丢掉。全名 `{YanYin}` 也照样认（原版 JP-Word 的写法）。",
    ],
    code: "5 5 5 5 |{yy}1 - - - |]",
  },
  {
