...
const EXPECT = {
  ...
  pages: 5,
  // 550 → 553：`.jpwabc` 的线型从前只写进 `BarlineEntry`、排版层不认（见
  // layout.ts::Line.load 那段注释），于是示例谱末尾那根 `|]` 只有**最后一页**画成终止线
  // ——前三页靠的是 `final` 标志，非末页就退成一根普通细线。现在逐页都照原文画，
  // 每页多一根粗线，共 +3。原样档不受影响（那根 `|]` 本来就落在末小节上）。
  leaves: 553,
